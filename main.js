...
// main function to handle calculation logic
const calculate = btnValue => {
    ...
    if (btnValue === '=') {
        ...
        const formatedInput = replaceOpertators(input)

        try {
            const calculatedValue = input.includes('%') ? calculatePercentage(input) : eval(formatedInput)
...
const replaceOpertators = input => input.replaceAll('÷', '/').replaceAll('×', '*')
...
const calculatePercentage = (input) => {
    ...
    return eval(replaceOpertators(processedInput))
}
